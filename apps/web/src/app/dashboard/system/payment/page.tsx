...
import { systemPaymentSettings } from '@/lib/api'
import {
  CreditCard,
  Percent,
  Info,
...
export default function PaymentSettingsPage() {
  ...
  const sp = t.systemPaymentSettings
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true)
      const all = await systemPaymentSettings.listAll()
      setStates((prev) => {
        const next = { ...prev }
  ...
  async function handleSave() {
    ...
    try {
      ...
      await systemPaymentSettings.upsert(activeTab, { isEnabled: current.isEnabled, config })
  ...
  async function handleFetchChipPublicKey() {
    ...
    setFetchingKey(true)
    try {
      const { publicKey } = await systemPaymentSettings.fetchChipPublicKey(secretKey, environment)
      setStates((prev) => ({
        ...prev,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{sp.title}</h1>
          <p className="text-sm text-gray-500">{sp.subtitle}</p>
        </div>
      </div>
