...
export default function BrandsPage() {
  ...
  const handleCreateBrand = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t.brands.failedCreate)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleSaveBranding = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(t.brands.brandingSaved)
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Gagal menyimpan branding')
      setError(e instanceof Error ? e.message : t.brands.failedSave)
    } finally {
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">{t.brands.subtitle}</p>
      </div>
        ...
        <form onSubmit={handleSaveBranding} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <label className="block text-sm font-medium text-gray-700 mb-1">{t.brands.companyName}</label>
              <input
                value={branding.companyName}
                onChange={e => setBranding(b => ({ ...b, companyName: e.target.value }))}
                  placeholder={t.brands.companyPlaceholder}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Warna Utama</label>
                          <label className="block text-sm font-medium text-gray-700 mb-1">{t.brands.primaryColor}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t.brands.logoUrl}</label>
              <input
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">URL Favicon</label>
                          <label className="block text-sm font-medium text-gray-700 mb-1">{t.brands.faviconUrl}</label>
              <input
            ...
            <div className="col-span-2">
              ...
              <label className="block text-sm font-medium text-gray-700 mb-1">{t.brands.emailHeader}</label>
              <textarea
                value={branding.emailHeader ?? ''}
                onChange={e => setBranding(b => ({ ...b, emailHeader: e.target.value }))}
                  placeholder={t.brands.emailHeaderPlaceholder}
                rows={3}
            ...
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">CSS Tersuai</label>
                          <label className="block text-sm font-medium text-gray-700 mb-1">{t.brands.customCss}</label>
              <textarea
            ...
            <div className="flex items-end gap-3">
              ...
              <div className="flex-1">
                <label className="block text-sm font-medium text-gray-700 mb-1">URL Logo (pilihan)</label>
                              <label className="block text-sm font-medium text-gray-700 mb-1">{t.brands.brandLogo}</label>
                <input
                  value={newBrandLogo}
        ...
        {items.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="font-medium">Tiada brand</p>
                      <p className="font-medium">{t.brands.noBrands}</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="text-left px-5 py-3 font-semibold text-gray-600">{t.brands.logo}</th>
                <th className="text-left px-5 py-3 font-semibold text-gray-600">{t.brands.status}</th>
                <th className="text-left px-5 py-3 font-semibold text-gray-600">{t.brands.created}</th>
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    <button onClick={() => toggleActive(item)} className="flex items-center gap-1.5 text-xs">
                      {item.isActive
                        ? <><ToggleRight size={18} className="text-green-500" /><span className="text-green-600">{t.brands.active}</span></>
                        : <><ToggleLeft size={18} className="text-gray-400" /><span className="text-gray-400">{t.brands.inactive}</span></>}
                    </button>
                  </td>
